...
async function onRequestPost(context) {
  try {
    ...
    console.log("\u{1F4E8} Recebido formul\xE1rio:", { email: formData.email, tipo: formData.tipo });
    if (!webhookUrl) {
      console.warn("\u26A0\uFE0F GOOGLE_SHEETS_WEBHOOK_URL n\xE3o est\xE1 configurada");
      return new Response(JSON.stringify({
        success: true,
        message: "Formul\xE1rio recebido! (Aviso: Webhook n\xE3o configurado)"
      }), {
        status: 200,
    ...
    if (response.ok) {
      ...
      return new Response(JSON.stringify({
        success: true,
        message: "Informa\xE7\xF5es enviadas com sucesso!"
      }), {
        status: 200,
        headers: { "Content-Type": "application/json" }
      });
...
// ../.wrangler/tmp/pages-PaAHhP/functionsRoutes-0.25881973879282527.mjs
var routes = [
  {
